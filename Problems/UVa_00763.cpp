...
BigInteger fib_to_dec(string fib){
    BigInteger result;
    result = 0;
    for(int i = fib.length()-1; i >= 0; i--){
        result = result + (fibonacci[fib.length()-i+1] * (fib[i]-'0'));
    }
    return result;
}

string dec_to_fib(BigInteger dec){
    string result = "";
    for(int i = 110; i > 1; i--){
        if(fibonacci[i] <= dec){
            result += '1';
...
int main(){
    ...
    while(cin >> a >> b){
    	if(first)
    		...
    		cout << endl;
        cout << dec_to_fib(fib_to_dec(a) + fib_to_dec(b)) << endl;
    }
}
